...
MIN_MATCH_COUNT = 6
MIN_HOMOGRAPHY_MATCHES = 5

class VisualSearch:
    def __init__(self, object_img_paths):
        # Use SIFT optimized for object detection with backgrounds
        self.detector = cv2.SIFT_create(
            nfeatures=4000, 
            contrastThreshold=0.04,  # Higher threshold filters weak wall features
            edgeThreshold=10,        # Lower threshold keeps object edges
            sigma=1.6               # Good for textured objects
        )
    ...
    def search_object(self, camera_frame):
        ...
        if best_ref_data and len(best_matches) > 0:
            result_img = cv2.drawMatches(best_ref_data['image'], best_ref_data['keypoints'], 
                                       camera_frame, frame_kp, best_matches[:20], None, flags=2)
